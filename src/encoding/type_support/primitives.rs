...
macro_rules! impls_for_tuple{
    (($($letters: ident), *), ($($numbers: tt), *) $(,) ?) => {
        impl <$($letters,) *> ForOverwrite <(),
        ($($letters,) *) > for() where $((): ForOverwrite <(), $letters >,) * {
            #[inline] fn for_overwrite() ->($($letters,) *) {
                ($(<() as ForOverwrite <(), $letters >>:: for_overwrite(),) *)
            }
        }
        impl <$($letters,) *> EmptyState <(),
        ($($letters,) *) > for() where $((): EmptyState <(), $letters >,) * {
            #[inline] fn empty() ->($($letters,) *) {
                ($(<() as EmptyState <(), $letters >>:: empty(),) *)
            }
            #[inline] fn is_empty(val: &($($letters,) *)) -> bool {
                true $(&& <() as EmptyState <(), $letters >>:: is_empty(& val.$numbers)) *
            }
            #[inline] fn clear(val: & mut($($letters,) *)) {
