...
#[doc = " Maps are always length delimited."]
...
macro_rules! impl_decoders{
    (
        ...
        V,
        KE,
        VE > $relaxed_value <$($lifetime,) ? Map < KE,
        VE >,
        M > for() where M: Mapping < Key = K,
        Value = V >,
        (): EmptyState <(),
        M > + ForOverwrite < KE,
        K > + ForOverwrite < VE,
        V > + $relaxed_value <$($lifetime,) ? KE,
        K > + $relaxed_value <$($lifetime,) ? VE,
        V >,
        {
            fn $relaxed_value_method $(
                $($buf_generic) *
            ) ?(value: & mut M, mut buf: Capped <$buf_ty >, ctx: DecodeContext,) -> Result <(),
            DecodeError > {
                ...
                while capped.has_remaining() ? {
                    let mut new_key = <() as ForOverwrite::< KE,
                    K >> ::for_overwrite();
                    let mut new_val = <() as ForOverwrite::< VE,
                    V >> ::for_overwrite();
                    <() as $relaxed_value < KE,
                    _ >>::$relaxed_value_method(&mut new_key, capped.lend(), ctx.clone()) ?;
                    <() as $relaxed_value < VE,
        ...
        V,
        KE,
        VE > $distinguished_value <$($lifetime,) ? Map < KE,
        VE >,
        M > for() where M: DistinguishedMapping < Key = K,
        Value = V > + Eq,
        K: Eq,
        V: Eq,
        (): EmptyState <(),
        M > + ForOverwrite < KE,
        K > + ForOverwrite < VE,
        V > + $distinguished_value <$($lifetime,) ? KE,
        K > + $distinguished_value <$($lifetime,) ? VE,
        V >,
        {
            ...
            DecodeError > {
                ...
                while capped.has_remaining() ? {
                    ...
                    V >> ::for_overwrite();
                    canon.update(
                        <() as $distinguished_value < KE,
                        _ >>::$distinguished_value_method::< true >(&mut new_key, capped.lend(), ctx.clone()) ?,
                    );
                    canon.update(
                        <() as $distinguished_value < VE,
...
mod test {
    mod btree {
        mod general {
            ...
            check_type_test!(Map, relaxed, BTreeMap < u64, f32 >, WireType::LengthDelimited);

            check_type_test!(Map, distinguished, BTreeMap < u32, i32 >, WireType::LengthDelimited);
        }
        ...
        mod fixed {
            ...
            check_type_test!(Map < Fixed, Fixed >, relaxed, BTreeMap < u64, f32 >, WireType::LengthDelimited);

            check_type_test!(Map < Fixed, Fixed >, distinguished, BTreeMap < u32, i32 >, WireType::LengthDelimited);
        }
        ...
        mod delegated_from_general {
            ...
            check_type_test!(General, relaxed, BTreeMap < bool, f32 >, WireType::LengthDelimited);

            check_type_test!(General, distinguished, BTreeMap < bool, u32 >, WireType::LengthDelimited);
        }
    }
}
