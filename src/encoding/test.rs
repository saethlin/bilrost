...
#[doc = " Generalized proptest macro. Kind must be either `relaxed` or `distinguished`."]
macro_rules! check_type_test{
    ($encoder: ty, $kind: ident, $ty: ty, $wire_type: expr) => {
        crate::encoding::test::check_type_test!($encoder, $kind, from $ty, into $ty, converter(value) {
            value
        }, $wire_type, check empty state);
    };
    ($encoder: ty, $kind: ident, from $from_ty: ty, into $into_ty: ty, $wire_type: expr) => {
        crate::encoding::test::check_type_test!($encoder, $kind, from $from_ty, into $into_ty, converter(value) {
            <$into_ty >:: from(value)
        }, $wire_type, check empty state);
    };
    ...
    ($encoder: ty, $kind: ident, from $from_ty: ty, into $into_ty: ty, $wire_type: expr, has no empty state) => {
        crate::encoding::test::check_type_test!($encoder, $kind, from $from_ty, into $into_ty, converter(value) {
            <$into_ty >:: from(value)
        }, $wire_type);
    };
    (
        ...
        $wire_type: expr $(, $check_fn_name: ident empty state) ?
    ) => {
        #[cfg(test)] mod $kind {
            ...
            use super::*;
            proptest!{
                $(#[test] fn $check_fn_name($from_value: $from_ty, tag: u32) {
                    check_type::<$into_ty,
                    $encoder >($convert, tag, $wire_type) ?;
                }) ? #[test] fn check_optional(opt_value: Option <$from_ty >, tag: u32) {
                    check_type::< Option <$into_ty >,
                    $encoder >(opt_value.map(|$from_value | $convert), tag, $wire_type,) ?;
                }
            }
        }
    };
...
macro_rules! check_borrowable{
    ...
    (borrowed: $ty: ty, encoding: $encoding: ty, mod $mod_name: ident $(,) ?) => {
        crate::encoding::test::check_borrowable!(
            borrowed: $ty,
            encoding: $encoding,
            mod $mod_name,
    ...
    (borrowed: $ty: ty, encoding: $encoding: ty, converter($from_value: ident: $from_ty: ty) $convert: expr $(,) ?) => {
        crate::encoding::test::check_borrowable!(
            borrowed: $ty,
            encoding: $encoding,
            mod borrow_equivalence,
    ...
    (
        ...
        mod $mod_name {
            ...
            proptest!{
                #[test] fn check($from_value: $from_ty) {
                    let val = $convert;
                    let mut buf = Vec::new();
                    <() as ValueEncoder <$encoding,
                    _ >> ::encode_value(&val, &mut buf);
                    let mut borrowed = <() as EmptyState <(),
                    &$ty >>:: empty();
                    <() as ValueBorrowDecoder <$encoding,
                    _ >>
                        ::borrow_decode_value(
                            &mut borrowed,
                    ...
                    prop_assert_eq!(borrowed, Borrow::<$ty >:: borrow(&val),);
                    let mut borrowed = <() as EmptyState <(),
                    &$ty >>:: empty();
                    prop_assert_eq!(
                        <() as DistinguishedValueBorrowDecoder <$encoding,
                        _ >>
...
macro_rules! check_type{
    ($kind: ident, $decoder_trait: ident, $context: expr, $decode: ident) => {
        pub mod $kind {
            ...
            pub fn check_type < T,
            E >(value: T, tag: u32, wire_type: WireType) -> TestCaseResult where T: Debug + PartialEq,
            (): ForOverwrite < E,
            T > + $decoder_trait < E,
            T >,
            {
                let expected_len = <() as Encoder < E,
                T >> ::encoded_len(tag, &value, &mut RuntimeTagMeasurer::new());
                let mut forward_encoded = Vec::with_capacity(expected_len);
                <() as Encoder < E,
                T >> ::encode(tag, &value, &mut forward_encoded, &mut TagWriter::new());
                prop_assert_eq!(expected_len, forward_encoded.len(), "forward encoded length was wrong");
                let mut prepend_buf = ReverseBuffer::new();
                let mut trw = TagRevWriter::new();
                <() as Encoder < E,
                T >> ::prepend_encode(tag, &value, &mut prepend_buf, &mut trw);
                trw.finalize(&mut prepend_buf);
                prop_assert_eq!(expected_len, prepend_buf.len(), "prepend encoded length was wrong");
                ...
                for encoded in[forward_encoded, prepended] {
                    ...
                    let mut roundtrip_value = <() as ForOverwrite < E,
                    T >> ::for_overwrite();
                    _ = <() as $decoder_trait < E,
                    T >>::$decode(wire_type, &mut roundtrip_value, buf.lend(), $context,) ?;
                    prop_assert!(
                        !buf.remaining() > 0,
            ...
            #[
                doc = " Helper function for `check_type` to disambiguate the variant of GeneralGeneric so"
            ] #[
                doc = " that this test helper can still be called without explicit generics."
            ] #[
                allow(dead_code)
            ] pub fn check_type_general < T >(
                value: T,
                tag: u32,
                wire_type: WireType
            ) -> TestCaseResult where T: Debug + Clone + PartialEq,
            (): ForOverwrite < General,
            T > + $decoder_trait < General,
            T > + ForOverwrite < GeneralPacked,
            T > + $decoder_trait < GeneralPacked,
            T >,
            {
                check_type::<T, General>(
                    value.clone(),
                    tag,
                    wire_type,
                ).or(check_type::<T, GeneralPacked>(value, tag, wire_type))
            }
            pub fn check_type_unpacked < T,
            E >(value: T, tag: u32, wire_type: WireType,) -> TestCaseResult where T: Debug + PartialEq,
            (): ForOverwrite < E,
            T > + $decoder_trait < E,
            T >,
...
#[doc = " Types that have an empty state should have a consistent implementation; this can be"]
#[doc = " especially important for types that use proxied encoding, which may want to keep emptiness"]
#[doc = " of the original and proxy types in sync. That's what this tests right now, though it may be"]
#[doc = " possible that this could be loosened somewhat in the future. There's some possibility that"]
#[doc = " eventually EmptyState may become subsidiary to value encoding rather than an enforced global"]
#[doc = " semantic, but we don't need to get ahead of ourselves."]
...
proptest! {
    ...
    #[test] fn u32_out_of_range(value in u32:: MAX as u64 + 1..) {
        let mut buf = Vec::<u8>::new();
        <() as ValueEncoder<General, _>>::encode_value(&value, &mut buf);
        let mut out = 0u32;
        prop_assert_eq!(
    ...
    #[test] fn i32_out_of_range(low_value in..i32:: MIN as i64 - 1, high_value in i32:: MAX as i64 + 1..,) {
    ...
    #[test] fn u16_out_of_range(value in u16:: MAX as u64 + 1..) {
        let mut buf = Vec::<u8>::new();
        <() as ValueEncoder<General, _>>::encode_value(&value, &mut buf);
        let mut out = 0u16;
        prop_assert_eq!(
    ...
    #[test] fn i16_out_of_range(low_value in..i16:: MIN as i64 - 1, high_value in i16:: MAX as i64 + 1..,) {
        for value in [low_value, high_value] {
            let mut buf = Vec::<u8>::new();
            <() as ValueEncoder<General, _>>::encode_value(&value, &mut buf);
            let mut out = 0i16;
    ...
    #[test] fn u8_out_of_range(value in u8:: MAX as u64 + 1..) {
        let mut buf = Vec::<u8>::new();
    ...
    #[test] fn i8_out_of_range(low_value in..i8:: MIN as i64 - 1, high_value in i8:: MAX as i64 + 1..,) {
        for value in [low_value, high_value] {
            let mut buf = Vec::<u8>::new();
            <() as ValueEncoder<Varint, _>>::encode_value(&value, &mut buf);
            let mut out = 0i8;
    ...
    #[test] fn bool_out_of_range(varint in 2u64..) {
        let mut buf = Vec::<u8>::new();
    ...
    #[test] fn field_key_too_big(tag in u32:: MAX as u64 + 1..) {
        let mut buf = Vec::<u8>::new();
